    ...
    <main style={{ background: "#0f1117", minHeight: "100vh", color: "#f9fafb", fontFamily: "'Inter', 'Poppins', sans-serif" }}>
      ...
      <section style={{ background: "linear-gradient(160deg,#0f1117 0%,#1a2035 100%)", padding: "5rem 2rem 4rem", borderBottom: "1px solid #1e2539" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <p style={{ margin: "0 0 0.5rem", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", color: "#6b7280" }}>Hjelp</p>
      ...
      <section style={{ padding: "3rem 2rem 0", borderBottom: "1px solid #1e2539" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <p style={{ margin: "0 0 0.5rem", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", color: "#6b7280" }}>Populære temaer</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: "1rem", padding: "1.25rem 0 2.5rem" }}>
            {[
              ...
              <div key={label} style={{ background: "#111827", border: "1px solid #1e2539", borderRadius: 4, padding: "1.1rem 1.25rem", display: "flex", alignItems: "center", gap: "0.75rem", cursor: "default" }}>
                <span style={{ fontSize: "1.3rem", flexShrink: 0 }}>{icon}</span>
                <span style={{ fontSize: "0.88rem", fontWeight: 700, color: "#d1d5db" }}>{label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "4rem 2rem" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 320px", gap: "3rem", alignItems: "start" }}>
          <div>
            <p style={{ margin: "0 0 0.5rem", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", color: "#6b7280" }}>FAQ</p>
            ...
            <div style={{ display: "grid", gap: "0.65rem" }}>
              {FAQ.map((item, i) => (
                <div
                  key={i}
                  style={{ background: "#111827", border: `1px solid ${openIndex === i ? "#4f6ef7" : "#1e2539"}`, borderRadius: 4, overflow: "hidden", transition: "border-color 0.15s" }}
                >
                  <button
                    onClick={() => setOpenIndex(openIndex === i ? null : i)}
                    style={{ width: "100%", padding: "1.1rem 1.3rem", background: "transparent", border: "none", color: "#f9fafb", textAlign: "left", cursor: "pointer", fontWeight: 700, fontSize: "0.92rem", display: "flex", justifyContent: "space-between", alignItems: "center", gap: "1rem", fontFamily: "inherit" }}
                  >
                    <span>{item.q}</span>
                    <span style={{ fontSize: "1.1rem", color: "#6b7280", flexShrink: 0, transform: openIndex === i ? "rotate(45deg)" : "none", transition: "transform 0.15s" }}>+</span>
                  </button>
                  {openIndex === i && (
                    <div style={{ padding: "0 1.3rem 1.1rem", color: "#9ca3af", fontSize: "0.9rem", lineHeight: 1.7 }}>
                      {item.a}
                    </div>
                  )}
                </div>
          ...
          <div style={{ display: "grid", gap: "1rem" }}>
            <div style={{ background: "#111827", border: "1px solid #1e2539", borderRadius: 4, padding: "1.5rem" }}>
              <h3 style={{ margin: "0 0 0.75rem", fontSize: "0.95rem", fontWeight: 800, color: "#f9fafb" }}>Fant du ikke svaret?</h3>
              <p style={{ margin: "0 0 1.25rem", fontSize: "0.88rem", color: "#9ca3af", lineHeight: 1.65 }}>
                Send oss en melding direkte og vi svarer deg innen 1–2 virkedager.
              </p>
              <a href="/kontakt" style={{ display: "block", padding: "0.85rem", background: "#fff", color: "#111827", borderRadius: 4, textDecoration: "none", fontWeight: 800, fontSize: "0.88rem", textAlign: "center", letterSpacing: "0.02em" }}>
                Kontakt oss →
              </a>
            </div>

            <div style={{ background: "#111827", border: "1px solid #1e2539", borderRadius: 4, padding: "1.5rem" }}>
              <h3 style={{ margin: "0 0 0.75rem", fontSize: "0.95rem", fontWeight: 800, color: "#f9fafb" }}>Åpningstider</h3>
              <div style={{ display: "grid", gap: "0.5rem" }}>
                {[
                  ...
                  <div key={day} style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem" }}>
                    ...
                    <span style={{ fontWeight: 700, color: "#f9fafb" }}>{hours}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ background: "#111827", border: "1px solid #1e2539", borderRadius: 4, padding: "1.5rem" }}>
              <h3 style={{ margin: "0 0 0.75rem", fontSize: "0.95rem", fontWeight: 800, color: "#f9fafb" }}>Nødtelefon</h3>
              <p style={{ margin: "0 0 0.5rem", fontSize: "0.85rem", color: "#9ca3af" }}>Tekniske problemer med aktiv booking?</p>
              <div style={{ fontSize: "1.1rem", fontWeight: 900, color: "#f9fafb", letterSpacing: "0.02em" }}>+47 22 00 00 00</div>
            </div>
          </div>
        </div>
      </section>
