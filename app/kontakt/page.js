    ...
    <main style={{ background: "var(--bg)", minHeight: "100vh", color: "var(--text)", fontFamily: "'Inter', 'Poppins', sans-serif" }}>
      ...
      <section style={{ background: "linear-gradient(160deg,var(--bg) 0%,#1a2035 100%)", padding: "5rem 2rem 4rem", borderBottom: "1px solid #1e2539" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <p style={{ margin: "0 0 0.5rem", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.15em", color: "var(--text-muted)" }}>Kontakt</p>
      ...
      <section style={{ padding: "4rem 2rem" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 380px", gap: "2.5rem", alignItems: "start" }}>
          ...
          <div>
            ...
            {sent ? (
              <div style={{ background: "var(--bg-panel)", border: "1px solid #6675ff", borderRadius: 4, padding: "2rem", textAlign: "center" }}>
                <div style={{ fontSize: "2.5rem", marginBottom: "0.75rem" }}>✓</div>
                <div style={{ fontWeight: 800, fontSize: "1.1rem", marginBottom: "0.5rem" }}>Meldingen er sendt!</div>
                <div style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>Vi vil kontakte deg på {form.email} snart.</div>
              </div>
            ) : (
              <form onSubmit={handleSubmit} style={{ display: "grid", gap: "1rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                  <div>
                    <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-muted)", marginBottom: "0.4rem" }}>Navn</label>
                    <input required name="name" value={form.name} onChange={handleChange} placeholder="Ola Nordmann" style={inputStyle} />
                  </div>
                  <div>
                    <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-muted)", marginBottom: "0.4rem" }}>E-post</label>
                    <input required type="email" name="email" value={form.email} onChange={handleChange} placeholder="ola@eksempel.no" style={inputStyle} />
                  </div>
                </div>
                <div>
                  <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-muted)", marginBottom: "0.4rem" }}>Emne</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-muted)", marginBottom: "0.4rem" }}>Melding</label>
                ...
                <button type="submit" style={{ padding: "0.9rem 2rem", background: "#fff", color: "var(--bg-panel)", border: "none", borderRadius: 4, fontWeight: 800, fontSize: "0.9rem", cursor: "pointer", letterSpacing: "0.02em", alignSelf: "start" }}>
                  Send melding →
                </button>
              </form>
            )}
          ...
          <div style={{ display: "grid", gap: "1rem" }}>
            ...
            {[
              ...
              <div key={label} style={{ background: "var(--bg-panel)", border: "1px solid #1e2539", borderRadius: 4, padding: "1.1rem 1.25rem", display: "flex", gap: "1rem", alignItems: "flex-start" }}>
                ...
                <div>
                  <div style={{ fontSize: "0.7rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-muted)", marginBottom: "0.2rem" }}>{label}</div>
                  <div style={{ fontSize: "0.92rem", fontWeight: 600, color: "var(--text)" }}>{value}</div>
                </div>
              </div>
            ))}

            {/* Sosialemedier */}
            <div style={{ background: "var(--bg-panel)", border: "1px solid #1e2539", borderRadius: 4, padding: "1.1rem 1.25rem" }}>
              <div style={{ fontSize: "0.7rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-muted)", marginBottom: "0.75rem" }}>Sosiale medier</div>
              <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap" }}>
                {["Instagram", "Facebook", "Twitter"].map((s) => (
                  <span key={s} style={{ padding: "0.35rem 0.85rem", background: "var(--bg-panel)", border: "1px solid #252d40", borderRadius: 4, fontSize: "0.8rem", color: "var(--text-muted)", fontWeight: 600 }}>{s}</span>
                ))}
              </div>
            </div>
          </div>
