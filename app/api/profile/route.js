import { NextResponse } from "next/server";
import { cookies } from "next/headers";
import pool from "../../../lib/db";

const COOKIE_NAME = "sid";

function noStoreJson(body, status) {
  const res = NextResponse.json(body, { status });
  res.headers.set("Cache-Control", "no-store, max-age=0, must-revalidate");
  res.headers.set("Pragma", "no-cache");
  res.headers.set("Expires", "0");
  res.headers.set("Vary", "Cookie");
  return res;
}

async function getUserIdFromSession() {
  const cookieStore = await cookies();
  const sid = cookieStore.get(COOKIE_NAME)?.value;
  if (!sid) return null;

  const sessRes = await pool.query(
    `SELECT sess, expire FROM public."session" WHERE sid = $1`,
    [sid]
  );
  if (sessRes.rowCount === 0) return null;

  const row = sessRes.rows[0];
  if (new Date(row.expire).getTime() <= Date.now()) {
    await pool.query(`DELETE FROM public."session" WHERE sid = $1`, [sid]);
    return null;
  }

  return row.sess?.userId ?? null;
}

async function buildProfileObject(user) {
  if (!user) return null;

  const normalizedDob = user.dob
    ? new Date(user.dob).toISOString().slice(0, 10)
    : "";

  let parsedInterests = [];
  if (Array.isArray(user.interests)) {
    parsedInterests = user.interests;
  } else if (typeof user.interests === "string" && user.interests.trim()) {
    try {
      const maybeArray = JSON.parse(user.interests);
      parsedInterests = Array.isArray(maybeArray) ? maybeArray : [];
    } catch {
      parsedInterests = [];
    }
  }

  return {
    id: user.id,
    name: user.name || user.username || "Bruker",
    email: user.email || "",
    phone: user.phone || "",
    dob: normalizedDob,
    age: user.age || "",
    bio: user.bio || "",
    profileImage: user.profile_image || user.avatar || "/images/fjell.jpg",
    bannerImage: user.banner_image || "/images/profilbakgrunn.jpg",
    lastTrip: {
      title: "Ingen tur registrert",
      date: "",
      description: "Ingen turdata tilgjengelig.",
      image: "/images/fjell.jpg",
      difficulty: "Ukjent",
      distance: "Ukjent",
      duration: "Ukjent",
      elevation: "Ukjent",
      rating: 0,
      location: "",
    },
    transactions: [],
    payment: { card: "Ikke registrert", billing: "Ingen fakturaperiode" },
    settings: {
      notifications: user.notifications !== false,
      theme: user.theme || "Lys",
    },
    interests: parsedInterests,
    radius_km: user.radius_km || 50,
  };
}

export async function GET(req) {
  try {
    const userId = await getUserIdFromSession();
    if (!userId) return noStoreJson({ error: "Ikke innlogget" }, 401);

    const result = await pool.query(
      "SELECT * FROM public.users WHERE id = $1",
      [userId]
    );

    if (result.rowCount === 0) {
      return noStoreJson({ error: "Bruker ikke funnet" }, 404);
    }

    const profile = await buildProfileObject(result.rows[0]);

    return noStoreJson({ profile }, 200);
  } catch (err) {
    console.error("GET /api/profile error:", err);
    return noStoreJson({ error: "Kunne ikke hente profil" }, 500);
  }
}

export async function PUT(req) {
  try {
    const userId = await getUserIdFromSession();
    if (!userId) return noStoreJson({ error: "Ikke innlogget" }, 401);

    const body = await req.json();
    
    // Extract fields to update
    const {
      name,
      email,
      phone,
      dob,
      age,
      bio,
      profileImage,
      bannerImage,
      interests,
      radius_km,
    } = body;

    // Validation
    if (name && String(name).trim().length > 100) {
      return noStoreJson({ error: "Navn er for langt" }, 400);
    }
    if (email && String(email).trim().length > 100) {
      return noStoreJson({ error: "E-post er for lang" }, 400);
    }
    if (phone && String(phone).trim().length > 20) {
      return noStoreJson({ error: "Telefonnummer er for langt" }, 400);
    }
    if (bio && String(bio).trim().length > 500) {
      return noStoreJson({ error: "Bio er for lang (maks 500)" }, 400);
    }

    // Build dynamic UPDATE statement
    const updates = [];
    const values = [userId];
    let paramCount = 2;

    if (name !== undefined) {
      updates.push(`name = $${paramCount}`);
      values.push(String(name).trim());
      paramCount++;
    }

    if (email !== undefined) {
      updates.push(`email = $${paramCount}`);
      values.push(String(email).trim());
      paramCount++;
    }

    if (phone !== undefined) {
      updates.push(`phone = $${paramCount}`);
      values.push(String(phone).trim());
      paramCount++;
    }

    if (dob !== undefined) {
      updates.push(`dob = $${paramCount}`);
      values.push(dob ? String(dob).trim() : null);
      paramCount++;
    }

    if (age !== undefined) {
      updates.push(`age = $${paramCount}`);
      values.push(age ? parseInt(age) : null);
      paramCount++;
    }

    if (bio !== undefined) {
      updates.push(`bio = $${paramCount}`);
      values.push(String(bio).trim());
      paramCount++;
    }

    if (profileImage !== undefined) {
      updates.push(`profile_image = $${paramCount}`);
      values.push(String(profileImage).trim());
      paramCount++;
    }

    if (bannerImage !== undefined) {
      updates.push(`banner_image = $${paramCount}`);
      values.push(String(bannerImage).trim());
      paramCount++;
    }

    if (interests !== undefined && Array.isArray(interests)) {
      updates.push(`interests = $${paramCount}`);
      values.push(JSON.stringify(interests));
      paramCount++;
    }

    if (radius_km !== undefined) {
      updates.push(`radius_km = $${paramCount}`);
      values.push(parseInt(radius_km) || 50);
      paramCount++;
    }

    if (updates.length === 0) {
      return noStoreJson({ error: "Ingen felter å oppdatere" }, 400);
    }

    const query = `UPDATE public.users SET ${updates.join(", ")} WHERE id = $1 RETURNING *`;
    const result = await pool.query(query, values);

    if (result.rowCount === 0) {
      return noStoreJson({ error: "Bruker ikke funnet" }, 404);
    }

    const profile = await buildProfileObject(result.rows[0]);

    return noStoreJson({ profile }, 200);
  } catch (err) {
    console.error("PUT /api/profile error:", err);
    return noStoreJson({ error: "Kunne ikke oppdatere profil" }, 500);
  }
}
